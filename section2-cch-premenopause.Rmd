...
df.pc$fertn.cutq = cut(df.pc$fertn.log, qrts, include.lowest = T, dig.lab=2)
...
df.pc$fesat.qrt = cut(df.pc$fesat, qrts, include.lowest = T, dig.lab=2)
...
#  pc1
summary(df.pc$PC1)
dec = quantile(df.pc$PC1, seq(0,1,by=0.1), na.rm=T) # use sample for quartile calcs
df.pc$pc1.dec = cut(df.pc$PC1, dec, include.lowest = T, dig.lab=2)
summary(df.pc$pc1.dec)

# ferritin
dec = quantile(df.pc$fertn,seq(0,1,by=0.1), na.rm=T) # use sample for quartile calcs
df.pc$fertn.dec = cut(df.pc$fertn, dec, include.lowest = T, dig.lab=4)
summary(df.pc$fertn.dec)
```
...
df.fertn.km.pre = df.fertn.km

# save for reviewer-tables.Rmd
save(pval, km.plot, df.fertn.km.pre, file="prem-km-fertn.RData")
...
# run km strata function over deciles for fertn
names.fertn = levels(factor(ccdat.rev$fertn.dec)); names.fertn
ironvar2="fertn"
list.fertn.km = lapply(names.fertn, get.strata, ironvar="fertn.dec", ironvar2=ironvar2)
...
df.fe.km.pre = df.fe.km
# save for reviewer-tables.Rmd
save(pval, km.plot, df.fe.km.pre, file="prem-km-fe.RData")
...
df.fesat.km.pre = df.fesat.km
# save for reviewer-tables.Rmd
save(pval, km.plot, df.fesat.km.pre, file="prem-km-fesat.RData")
...
km.plot = ggplot(data=df.pc1.km,
       aes(x=time, y=km, colour=strata)) + 
  labs(title="Survival probability by quartiles of first principal \ncomponent of three primary iron measures.",
       x="Age (years)",
...
png("km-plot.png", width=800, height=800)
  km.plot
dev.off()
...
df.pc1.km.pre = df.pc1.km
# save for reviewer-tables.Rmd
save(pval, km.plot, df.pc1.km.pre, file="prem-km-pc1.RData")
...
names.pc1 = levels(factor(ccdat.rev$pc1.dec)); names.pc1
ironvar2="PC1"
list.pc1.km = lapply(names.pc1, get.strata, ironvar="pc1.dec", ironvar2=ironvar2)
