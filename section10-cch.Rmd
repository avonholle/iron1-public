...
with(df, table(miss.meno.age, menop.status)) # a few people are post-menopausal at baseline but no age at menopause. Put age at menopause at baseline age.
...
with(data.frame(df), paste0(formatC(round(exp(coef),2), format="f", flag='0', digits=2),
  ...
  # if you want to bold cells that have p-values < 0.05
   # with(data.frame(df),  ifelse(p.value < 0.05, # no Bonferroni correction
   #                              paste0("\\textbf{",
...
sub.label2 = paste0(sub.label, " (n=", cox.qrt$n, ", subcohort n=", cox.qrt$subcohort.size, ")"); sub.label2
...
# output coef with 95% ci
coefs.ca = get.ci(ca)[1]; coefs.ca
coefs.ca.p = formatC(round(ca[1,4],3), format="f", digits=3)

# output coef with 95% ci
coefs.lin.adj = get.ci(ca)[1]; coefs.lin.adj
...
save(k2.bmi, file="../sections/k2-bmi.RData")
```
