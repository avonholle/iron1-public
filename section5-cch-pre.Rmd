---
title: "Section 5: Post-hoc 2: Association between extreme transferrin saturation (TSAT) levels and breast cancer risk (case-cohort analysis). Premenopausal"
output:
  pdf_document:
    toc: no
...
load(file="data-pre.RData") # df.pc
...
var.list = c("start.age", "c.age.alt", "event.alt", "iron.var")

# 1) Cox ph with quartiles ==================================

cox1 = cch(Surv(start.age, c.age.alt, event.alt) ~ iron.var ,
             data=df[complete.cases(df[var.list]),],
             subcoh = ~subcohort,
             id = ~ PSID,
             cohort.size=n.size)
...
cox.dat2.pre = cox.dat
cox.dat2.pre


cox.dat2.pre$threshold.f = with(cox.dat2.pre, factor(threshold, 
                                         labels = c("$\\leq$ 20",
                                                    "$\\geq$ 45",
                                                    "$\\geq$ 50",
                                                    "$\\geq$ 55",
...
save(cox.dat2.pre, file="../sections/k5-pre.RData")
```
...
cox.dat2.pre =  coefs.list2 %>% bind_rows() # source: https://stackoverflow.com/questions/2641653/pass-a-data-frame-column-name-to-a-function
